# --------------------
# (12) plot wavelet
# *** this plots wavelet of SSB time series after CV standardized among spp
...
plot_wvlt_CVnotstd_AUC1 <- function(burn_in_pd = rm_first_timesteps, #remove from beginning of ts
  ...
  #                           ts.data$EI==0 &
  #                           ts.data$variable=="Nsize",]$value))
  tsylim <-c(5750,120000) # also based on eyeballing from code above
  specNlim <- c(min(ts.spec[ts.spec$EI==0 & ts.spec$variable=="Nsize",]$specN),0.025) # based on eyeballing
  speclim <- c(min(ts.spec[ts.spec$EI==0 & ts.spec$variable=="Nsize",]$spec),
  ...
  tiff(file=paste('C:/Users/Mikaela/Documents/GitHub/pfmc/results/wavelet_plots/1_A_CVnotstd_AUC1_',noisename,'.tiff',sep=""), 
  ...
  text(x=100,y=0.2,
       label=paste(spp[i],
                   ...
                   spawndistmetrics[spawndistmetrics$spp==spp[i],]$cvs_mode,"\npeak=",
                   spawndistmetrics[spawndistmetrics$spp==spp[i],]$mode_age,sep=""),cex=1.2,pos=2)
  ...
  # (C) plot wavelet power spectrum
  ts <- ts.data[ts.data$spp==spp[i] & 
                  ts.data$EI==0 & 
                  ts.data$noise==noisename &
                  ts.data$variable=="Nsize" &
                  ts.data$year %in% rows2plot,]$value
  wavept <- wt(cbind(1:num_rows2plt,ts), 
               dj = 0.01, J1 = J1, max.scale = 32, mother = "morlet", 
               sig.test = 0, sig.level = 0.95)
  rm(ts)
  plot(wavept)
  # (D) Plot frequency response
  plot(x=ts.spec[ts.spec$spp==spp[i] &
       ...
       y=ts.spec[ts.spec$spp==spp[i] &
                   ...
                   ts.spec$variable=="Nsize" &
                   ts.spec$EI==0,]$spec,
       xlim=c(0,0.5),ylim=speclim,type = "l",ylab="",
       xlab="frequency",log="y",yaxs="i")
  ...
  plot(x=ts.spec[ts.spec$spp==spp[i] &
       ...
       y=ts.spec[ts.spec$spp==spp[i] &
                   ...
                   ts.spec$variable=="Nsize" &
                   ts.spec$EI==0,]$specN,
       type = "l",ylab="",xlab="frequency",log="y",yaxs="i",ylim=specNlim)
  title(ylab="log(power)",line=-1)
  ...
  # --------
  tiff(file=paste('C:/Users/Mikaela/Documents/GitHub/pfmc/results/wavelet_plots/1_B_CVnotstd_AUC1_',noisename,'.tiff',sep=""), 
       units="in", width=8, height=11, res=300) 
  par(mfrow=c(7,4),mai=c(0.2,0.3,0.2,0.2))
