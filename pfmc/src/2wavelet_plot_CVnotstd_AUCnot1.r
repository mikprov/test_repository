...
plot_wvlt_CVnotstd_AUCnot1 <- function(burn_in_pd = rm_first_timesteps, #remove from beginning of ts
  ...
  tiff(file=paste('C:/Users/Mikaela/Documents/GitHub/pfmc/results/wavelet_plots/2_A_CVnotstd_AUCnot1_',noisename,'.tiff',sep=""), 
  ...
  plot(x=ts.spec[ts.spec$spp==spp[i] &
       ...
       y=ts.spec[ts.spec$spp==spp[i] &
                   ...
                   ts.spec$variable=="Nsize" &
                   ts.spec$EI==0,]$spec,
       type = "l",ylab="",xlab="frequency",log="y",yaxs="i")
  ...
  # --------
  tiff(file=paste('C:/Users/Mikaela/Documents/GitHub/pfmc/results/wavelet_plots/2_B_CVnotstd_AUCnot1_',noisename,'.tiff',sep=""), 
       units="in", width=8, height=11, res=300) 
  par(mfrow=c(7,4),mai=c(0.2,0.3,0.2,0.2))
